...
- **Stage-2 init: LE for all D by default; optional Mercator ordering-init for D=1**: the reference C++ uses LE only for D≥2 and the classic Mercator ordering + expected-angular-gap re-spacing for D=1. `DMercatorEmbedder.fit(..., d1_init=...)` / `dmercator_init(..., d1_init=...)` default to `"le"` (the paper's generalisation, used for all D); pass `d1_init="mercator"` to use `mercator_ordering_init` (the C++ D=1 init) instead. The flag is ignored (with a warning) for `d > 2`.
