...
Current contents:
- `estimate_gamma(G, k_min=None, fallback_gamma=DEFAULT_GAMMA)` — discrete power-law MLE (Clauset–Shalizi–Newman, arXiv:0706.1062, §3.2). `k_min=None` (default) selects the cutoff automatically by KS minimisation; an explicit integer uses that cutoff directly. When no cutoff can be selected (< 3 distinct degrees, not power-law), it warns and returns `fallback_gamma` (`DEFAULT_GAMMA = 2.5`, a neutral scale-free value; chosen over fitting because the `k_min=1` MLE can return an invalid `γ < 2`). Also importable as `hypegrl.embedders.hypermap.estimate_gamma`.
...
- `power_law_gof(degrees, n_bootstrap=1000, seed=None)` — CSN §4 semiparametric-bootstrap goodness-of-fit for the power-law hypothesis; **user-called, never automatic**. Returns `{p_value, plausible, D, k_min, gamma, n_tail, n_bootstrap}` with `plausible = p_value >= 0.1`. Small `p` **rejects** the power law; large `p` only means it's *plausible*, not correct/best. Rejects trees (`p≈0`), passes BA (`p≈0.5`); karate is "plausible" only because N=34 is too small to reject.
