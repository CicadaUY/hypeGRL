...
- **Node ordering in HyperMap**: `HyperMapEmbedder` sorts nodes by degree during the greedy initialization phase. The row order of `embeddings()` matches `embedder.nodes_sorted`, not the original NetworkX node order. Consequence (a real silent footgun): `plot_poincare_graph(G, emb.embeddings())` and any adjacency you build to pair with the rows must account for that order, otherwise every node is drawn at the wrong point and the plot looks like noise. `plot_poincare_graph` takes a `nodes=` argument for exactly this — pass `nodes=emb.nodes_sorted` and it aligns internally. `PoincareMapsEmbedder` keeps `G.nodes()` order (it uses `nx.to_numpy_array(G)`), so it needs no `nodes=`. Documented in the `plot_poincare_graph` and `HyperMapEmbedder.embeddings()` docstrings.
