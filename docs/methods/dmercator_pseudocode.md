...
Clustering is **monotone increasing in β**, so β is found by a **bidirectional**
bracket-bisection — raise β when the model *under*-clusters, lower it toward the floor `β = D`
when it *over*-clusters — and the κ inner loop is re-run for every β evaluated. There is a hard
lower bound `β = D` (the model is undefined for `β ≤ D`): low-clustering graphs (a tree has
`c̄_emp = 0`, which the S^D model cannot reach at finite β) drive β down to the floor `D + 0.01`,
where it is clamped with a warning.

> **Correction (provenance).** An earlier version of this section described only the
> *one-directional* case — `while c̄ < c̄_emp: grow β` — with no opposite branch, no `β = D`
...
function INFER_KAPPA_AND_BETA(A, k, ⟨k⟩, c̄_emp, R, D):
    β_floor = D + 0.01                                 # BETA_ABS_MIN_DIM; β must strictly exceed D
    β_ceil  = D + 100                                  # BETA_ABS_MAX_DIM
    β_min   = D                                        # hard lower bound
    β_max   = -1                                       # sentinel: no upper bracket established yet

    function EVAL(β):                                  # all model state recomputed at this β
        μ  = μ(β, D, ⟨k⟩)
    ...
    repeat at most N_MAX times:                         # N_MAX guards an unreachable target
        ...
        if c̄ > c̄_emp:                                  # over-clustered ⇒ lower β toward the floor
            ...
            β       = (β_max + β_min)/2
            clamped = (β < β_floor)
        else:                                           # under-clustered ⇒ raise β
            β_min   = β
            β       = 1.5*β  if β_max == -1  else (β_max + β_min)/2
            clamped = (β > β_ceil)

        # Recompute κ and c̄ at the NEW β, so the returned (κ, β) stay self-consistent even when we
        # clamp. (The C++ instead breaks reporting the clamped β alongside κ/μ from the previous
        # iterate — the origin of the μ "penultimate-β" mismatch documented in the equivalence
        # tests. This pseudo-code follows the self-consistent variant the init module implements.)
        (κ, c̄)  = EVAL(β)
        if clamped:
            warn "β driven to its bound; model clustering unreachable"
            break
